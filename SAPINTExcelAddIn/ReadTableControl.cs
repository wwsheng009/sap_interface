        ...
        void loadTableData()
        {
            try
            {
                ...
                if (!ws.Controls.Contains(_tableName))
                {
                    ...
                    ls = ws.Controls.AddListObject(ws.Range["A2"], _tableName);
                ...
                ls.SetDataBinding(dt.Result);
                ws.Rows[1].Clear();
                // ReadTableFieldCollection co = dt.Fields;
                for (int i = 0; i < dt.Fields.Count; i++)
                {
                    ws.Cells.set_Item(1, i + 1, dt.Fields[i].FieldText);
                    ls.ListColumns[i + 1].Name = dt.Fields[i].FieldName;
                ...
                ws.Deactivate += new Microsoft.Office.Interop.Excel.DocEvents_DeactivateEventHandler(ws_Deactivate);
